"""
This module contains tests for the CUID generation functionality.

To run the tests in this file individually, use the following command:
    pytest tests/test_cuids.py

To run all tests in the @tests/ folder, use:
    pytest tests/

Logs generated during the tests will be displayed in the console.
Ensure that the logging level is set to INFO to see the generated CUIDs and other log messages.
"""
...
def test_generate_multiple_cuids():
    ...
    """
    cuids = [
        generate_cuid() for _ in range(1000)
    ]  # Increased sample size for better duplicate detection
    assert len(cuids) == 1000
    unique_cuids = set(cuids)
    assert len(unique_cuids) == 1000, f"Found {
        len(cuids) - len(unique_cuids)} duplicate CUIDs"
